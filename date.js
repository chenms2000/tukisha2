...
async function requestCharToPay(scene, targetCharId) {
  ...
  const hiddenMessage = {
    role: 'system',
    content: `[系统指令：用户邀请你进行约会"${scene.name
...
async function requestToBorrowMoney(scene, dateTargetCharId, lenderChatId, amount) {
  ...
  const hiddenMessage = {
    role: 'system',
    content: `[系统指令：用户 "${myNickname}" 想向你借 ${amount.toFixed(2)} 金币，用于和 "${dateTargetChat.name}" 在 "${scene.name
...
`;
  ...
  try {
    if (typeof window.buildWorldBookContentByPosition === 'function') {
      const worldBookByPosition = window.buildWorldBookContentByPosition(chat, recentMessages, false);
      worldBookContext = worldBookByPosition?.all || '';
    } else {
      console.warn('[Dating] buildWorldBookContentByPosition 未加载，跳过世界书注入');
    }
  } catch (e) {
    console.warn('[Dating] 世界书注入失败，跳过：', e);
    worldBookContext = '';
  }
                        ...
                        <div class="fields-container">
                            ...
                            <div class="form-group" style="margin:0;">
                                ...
                                <div class="bg-upload-container">
                                    ...
                                    <input class="sprite-url-input" type="text" value="${sprite.url || ''
    }" placeholder="或粘贴URL">
                                </div>
                            </div>
...
async function deleteDatingHistoryRecord(record) {
  const confirmed = await showCustomConfirm(
    '删除约会记录',
    '确定要删除这条约会记录吗？此操作不可恢复。',
    { confirmButtonClass: 'btn-danger' },
  );
  if (!confirmed) return;
  ...
  // 兼容两种情况：
  // 1) 表有自增主键 id
  // 2) 以 timestamp 建了索引（你用 orderBy('timestamp')，说明 timestamp 至少是索引）:contentReference[oaicite:2]{index=2}
  if (record.id !== undefined && record.id !== null && record.id !== '') {
    await db.datingHistory.delete(record.id);
  } else {
    await db.datingHistory.where('timestamp').equals(record.timestamp).delete();
  }

  await renderDatingHistory(); // 删除后刷新列表
}
...
function createDatingHistoryCard(record) {
  ...
  cardContainer.innerHTML = `
                        <div class="dating-summary-card-inner">
                            <button class="dating-history-delete-btn" data-id="${record.id ?? ''}" data-timestamp="${record.timestamp}" title="删除这条记录">×</button>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('dating-history-list').addEventListener('click', async e => {
    // 1️⃣ 如果点的是删除按钮
    const delBtn = e.target.closest('.dating-history-delete-btn');
    if (delBtn) {
      e.stopPropagation();

      const idStr = delBtn.dataset.id;
      const record = {
        id: idStr !== '' ? Number(idStr) : '',
        timestamp: parseInt(delBtn.dataset.timestamp, 10),
      };

      await deleteDatingHistoryRecord(record);
      return;
    }
